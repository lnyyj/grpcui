...
import (
	...
	"github.com/lnyyj/grpcui/internal/resources/webform"
)

var webFormTemplate = template.Must(template.New("grpc web form").Parse(string(webform.Template())))

func SetWebFormTemplate(tmpl *template.Template) {
	webFormTemplate = tmpl
}
...
var webformScript []byte

func init() {
	webformScript = webform.Script()
}
...
func WebFormScript() []byte {
	return webformScript
}

func SetWebFormScript(script []byte) {
	webformScript = script
}
